...
export default function Sidebar() {
  ...
  return (
    <aside className="bg-white w-[240px] h-full shadow-[6px_4px_4px_rgba(0,0,0,0.25)]">
                          ...
                          <div className="flex flex-wrap">
                            <div className="  w-full pl-2">
                              <span className="text-[15px] font-medium text-secondaryFont">
                                {idx + 1}. {slot.type === "assignment" ? "Hozzárendelés képeken":
                                            slot.type === "grouping" ? "Csoportba rendezés" :
                                            slot.type === "pair" ? "Párkereső" :
                                            slot.type === "short" ? "Rövid válasz" : ""
                                            }
                              </span>
                            </div>
                            <div className="flex justify-between items-center w-full">
                              {idx <9 ? <div className="ml-6 text-[15px] text-gray">{slot.label}</div> : <div className="ml-8 text-[15px] text-gray">{slot.label}</div>}
                              <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    removeTask(slot.id);
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" onMouseDown={cancelCreate}>
          ...
          >
            <div className="text-lg font-medium mb-3">Új feladat létrehozása</div>

            <input
              className="w-full border-[1px] border-lightBorder rounded-lg px-3 py-2 outline-none"
