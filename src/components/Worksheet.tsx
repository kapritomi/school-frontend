...
export const Worksheet = ({
  ...
  return (
    <div className={`flex gap-[16px] transition-all duration-300ms ${id===selectedWorksheetId ? "w-[400px]" : "w-[197px]"}`}>
        <div
        ...
        </div>
      {
        id ===selectedWorksheetId && (
          <div className="flex flex-col gap-4 p-4 text-[20px]">
            <p onClick={()=>navigate(`/worksheetSolutions/${id}`)} className ="cursor-pointer" >Megoldások</p>
            <p className="cursor-pointer">Szerkesztés</p>
            <p className="text-alert cursor-pointer">Törlés</p>
          </div>
        )
      }
    </div>
