...
export default function StudentView(){
    ...
    const TASK_COMPONENTS: Record<number, React.ComponentType<{ task: TaskJson }>> = {
        1: Grouping,
        2: Pairing,
        3: Assignment,
        4: ShortAnswer,
        ...
        <div className='h-screen overflow-y-scroll snap-y snap-mandatory'>
            <div className='fixed top-0 left-1/2 -translate-x-1/2 z-50'>
                <div className="relative flex items-center justify-center">
                    ...
                    <Clock /> <span className='text-[30px] text-white font-medium mb-3'>{minutes}:{seconds.toString().padStart(2, "0")}</span>
                    </div>
                </div>
            </div>
            ...
            <Link to={'/createTask'}>vissza</Link>
                ...
                {tasksJson.tasks.map((task) => {
                    ...
                    <div key={task.id} className="snap-start h-screen flex flex-col">
                        <h2 className="task-padding font-semibold text-TaskTitle">{task.task_title}</h2>
                        <Component task={task} />
                    </div>
                    );
                })}
